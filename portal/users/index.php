			...
			<div class="content">
				<div class="row">
					<div class="col-md-6 col-sm-6 col-lg-6 col-xl-3">
						<?php
						// include('../connect.php');
						$result = $db->prepare("SELECT count(*) as total FROM emergency WHERE  victim_id = {$_SESSION['SESS_USERS_ID']} AND status = 'Resolvido'  ");
						$result->execute();
						...
						?>
							<div class="dash-widget">
								...
								<div class="dash-widget-info text-right">
									...
									<span class="widget-title1">Resolvido Emergency<i class="fa fa-check" aria-hidden="true"></i></span>
								</div>
							</div>
						<?php } ?>
					</div>
					...
					<!-- <div class="col-md-6 col-sm-6 col-lg-6 col-xl-3">
                    	<?php
						// include('../connect.php');
						$result = $db->prepare("SELECT count(*) as total FROM emergency WHERE  agency_id = {$_SESSION['SESS_USERS_ID']}  AND status = 'Resolvido' ");
						$result->execute();
						for ($i = 0; $row = $result->fetch(); $i++) {
						?>
                        <div class="dash-widget">
					...
					<div class="col-md-6 col-sm-6 col-lg-6 col-xl-3">
						<?php
						// include('../connect.php');
						$result = $db->prepare("SELECT count(*) as total FROM emergency WHERE  victim_id = {$_SESSION['SESS_USERS_ID']}  AND status = 'Pendente' ");
						$result->execute();
						for ($i = 0; $row = $result->fetch(); $i++) {
						?>
							<div class="dash-widget">
								...
								<div class="dash-widget-info text-right">
									<h3><?php echo $row['total']; ?></h3>
									<span class="widget-title4">Pendente <i class="fa fa-check" aria-hidden="true"></i></span>
								</div>
							</div>
						<?php } ?>
					</div>
